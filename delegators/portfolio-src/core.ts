...
export type MarketplacePurchase = {raw:string;lovelace:string;buyer:string;source:'marketplace'|'allocated-bundle'};
export type Detail = { tx_hash:string; tx_timestamp:number; fee:string; inputs:Io[]; outputs:Io[]; assets_minted?:Asset[]; marketplace_version?:number; marketplace_purchases?:Record<string,MarketplacePurchase> };
export type Counterparty = {address:string;lovelace:string;stakeAddress?:string|null;txHash?:string;txIndex?:number};
export type Fact = { hash:string; time:number; adaRaw:string; assets:Record<string,string>; decimals:Record<string,number>; feeRaw:string|null; internal:boolean; wallets:string[]; swapCandidate:boolean; counterpartyVersion?:number; externalOutputs?:Counterparty[]; externalInputs?:Counterparty[]; minted?:Record<string,string>; inputRefs?:string[]; ownedInputCount?:number; marketplaceVersion?:number; marketplacePurchases?:Record<string,MarketplacePurchase>; source?:Detail };
export type Acquisition = {raw:string;ada:number;time:number;paymentHash:string;source:'mint'|'linked-mint'|'linked-purchase'|'marketplace'|'allocated-bundle'|'confirmed'};
export type Acquisitions = Record<string,Record<string,Acquisition>>;
export type Market = { token_id:string; ticker?:string|null; decimals?:number|null; price_by_ada?:number|null; price_by_usd?:number|null; is_verified?:boolean|null; logo?:string|null; image?:string|string[]|null; image_url?:string|null; is_nft?:boolean; cached_image?:string; wayup_image?:string; wayup_floor_ada?:number; wayup_quoted_at?:string };
...
export function analyse(detail:Detail, addresses:Set<string>):Fact {
  ...
  const marketplacePurchases=Object.fromEntries(Object.entries(detail.marketplace_purchases||{}).filter(([id,p])=>addresses.has(p.buyer)&&assets[id]===p.raw&&!detail.inputs.some(i=>addresses.has(i.payment_addr?.bech32||'')&&i.asset_list?.some(a=>assetId(a)===id))));
  return {hash:detail.tx_hash,time:detail.tx_timestamp,adaRaw:String(ada),assets,decimals,minted,inputRefs,ownedInputCount,counterpartyVersion:1,marketplaceVersion:detail.marketplace_version,marketplacePurchases,feeRaw:allInputs?String(fee):null,internal:allInputs&&allOutputs&&Object.keys(assets).length===0&&ada===-fee,wallets:[...touched],externalOutputs,externalInputs,swapCandidate:detail.inputs.some(x=>!addresses.has(x.payment_addr?.bech32||''))||detail.outputs.some(x=>!addresses.has(x.payment_addr?.bech32||''))};
}
