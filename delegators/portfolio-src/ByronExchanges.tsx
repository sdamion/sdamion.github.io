...
import {discoveredByronAddresses,saveByronSelection} from './byron-exchanges';
...
export function ByronExchanges({facts,entries,owned,history,complete,onChange}:{facts:Record<string,Fact>;entries:CexAddress[];owned:string[];history:Record<string,number>;complete:boolean;onChange:(entries:CexAddress[])=>boolean}){
  ...
  const saved=new Set(group.map(entry=>entry.address));
  const selected=new Set(candidates.filter(row=>choices[row.address]??saved.has(row.address)).map(row=>row.address));
  ...
  return <section className="portfolio-section" aria-labelledby="portfolio-byron-title">
    ...
    <p className="small muted">Addresses found in your loaded transactions, not verified exchange ownership. Select only addresses you know belong to a CEX. Saved in this browser; these addresses are not added to your wallet balances.</p>
    <form className="portfolio-section" onSubmit={event=>{event.preventDefault();const next=saveByronSelection(entries,candidates.map(row=>row.address),selected,name,owned);if(onChange(next)){setChoices({});setStatus('Byron exchange selection saved.');}else setStatus('Could not save the selection.');}}>
      ...
      <div className="history-table"><Table><TableHeader><TableRow><TableHead>Byron address</TableHead><TableHead>Transactions</TableHead><TableHead>Last seen</TableHead><TableHead>CEX</TableHead></TableRow></TableHeader><TableBody>{filtered.slice(current*25,(current+1)*25).map(row=><TableRow key={row.address}>
        ...
        <TableCell><input type="checkbox" name="byron_cex_selection" aria-label={`Include ${row.address} in Byron CEX`} checked={selected.has(row.address)} onChange={event=>{const checked=event.target.checked;setChoices(previous=>({...previous,[row.address]:checked}));setStatus('');}}/></TableCell>
