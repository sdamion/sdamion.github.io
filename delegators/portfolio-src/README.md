...
Mint receipts retain Koios `assets_minted` in cached transaction facts. Old positive-asset receipts are reanalysed once on refresh without resetting the known transaction count. `mint-payments.ts` infers a price only for a single received minted asset with fully owned inputs and a net ADA spend matching external outputs. Fees and own change are excluded. Bundles and separate payments require a confirmed allocation under the asset's **Mint / purchase payment** control. Separate payments must be ADA-only outgoing transactions in the loaded history; allocations cannot exceed the payment or duplicate a receipt. Links are saved per member and wallet combination in this browser. FIFO acquisition costs and fallback historical quotes use the payment day's USD price, never a current quote substituted for a missing historical price. Mint prices are explicitly historical estimates, not live market valuations. Test with `node --experimental-strip-types mint-payments.test.ts`.
