...
type State={stake:string;mode:StorageMode;key:CryptoKey|null;data:Data;revision:string|null;expiresAt:number;dirty:boolean;generation:number;blocked:boolean;controller:AbortController};
...
export function lockVault(){unlockGeneration++;clearTimeout(timer);timer=undefined;state?.controller.abort();state=null;}
...
export async function unlockPortfolio(wallet:{signData:(payload:string,address:string)=>Promise<{signature:string;key:string}>},{role='delegator'}:{role?:'delegator'|'admin'}={}){
  ...
  if(stored.payload){
    try{data=await openVault(key,stake,stored.payload);}catch{throw new Error('This wallet approval could not unlock the saved Portfolio. Use the same wallet app and stake account. The cache has not been changed.');}
  ...
  check();
  state={stake,mode:'remote',key,data,revision:stored.revision,expiresAt:stored.expires_at,dirty:!stored.payload,generation:0,blocked:false,controller:new AbortController()};
...
function changed(){const current=active();current.dirty=true;current.generation++;if(!timer)timer=setTimeout(()=>{timer=undefined;void flushVault().catch(()=>{});},30000);}
...
export async function cacheSnapshot(name:string,data:Snapshot){const current=active();if(!name.startsWith(current.stake+'::'))throw new Error('Wrong Portfolio member.');current.data.snapshot={key:name,data};changed();}
export async function flushVault():Promise<void>{
  ...
  running=(async()=>{
    ...
    if(current!==state)return;
    const response=await portfolioFetch('/vault',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({revision:current.revision,payload}),signal:current.controller.signal});
    ...
    current.revision=result.revision;current.expiresAt=Math.max(current.expiresAt,result.expires_at);current.dirty=current.generation!==generation;
    await clearLegacy(current.stake);
    notice('Encrypted Portfolio cache saved.');
  })().catch(error=>{if(!current.controller.signal.aborted)notice(error.message);throw error;}).finally(()=>{running=null;});
  return running;
}
