...
import {short,validAddress} from './core';
...
export function CexAddresses({entries,owned,onChange}:{entries:CexAddress[];owned:string[];onChange:(entries:CexAddress[])=>boolean}){
  ...
  return <section className="portfolio-section"><h2>CEX addresses</h2>
    <p className="small muted">Your exchange deposit addresses. Labels are saved in this browser for this member. A deposit does not establish a sale.</p>
    <form className="wallet-form governance-drep-registration-form" onSubmit={event=>{
      ...
      if(!validAddress(value)){setError('Enter a valid mainnet payment address (addr1…).');return;}
      ...
      <label className="address-field" htmlFor="portfolio-cex-address">Deposit address<Input id="portfolio-cex-address" name="exchange_address" value={address} onChange={event=>setAddress(event.target.value)} required placeholder="addr1…"/></label>
    ...
    <div className="tdsp-tile-grid">{entries.map(entry=><div className="governance-menu-card" key={entry.address}>
      ...
      <a className="address" title={entry.address} href={`https://cardanoscan.io/address/${entry.address}`} target="_blank" rel="noreferrer">{short(entry.address)} <ExternalLink size={12}/></a>
    </div>)}</div>
  </section>;
}
