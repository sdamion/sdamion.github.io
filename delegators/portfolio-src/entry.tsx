...
function MemberPortfolio({wallet,role}:{wallet:()=>SigningWallet|null;role:'delegator'|'admin'}){
  ...
  useEffect(()=>{
    ...
    portfolioFetch('/session',{signal:control.signal}).then(async r=>{
      ...
      if(saved&&(saved==='remote'||!isPortfolioMobile())){
        setBusy(true);
        try{await switchStorage(saved,member,wallet(),role,false);if(!control.signal.aborted)setReady(true);}
  ...
  async function apply(mode:StorageMode,copy:boolean){setBusy(true);setError('');try{await new Promise(resolve=>setTimeout(resolve,0));await switchStorage(mode,stake,wallet(),role,copy);setPreference(mode);setReady(true);setSettings(false);setVersion(value=>value+1);}catch(e){setError(e instanceof Error?e.message:'Storage change failed.');setReady(vaultUnlocked(stake));}finally{setBusy(false);}}
  ...
  return <><section className="member-portfolio"><div className="portfolio-body">{ready||preference?<>
    ...
    {!ready&&!settings&&<div role="status"><p>{busy?(mode==='remote'?'Unlocking encrypted Portfolio cache. Approve the request in your wallet.':'Opening local Portfolio cache…'):error||'Unlock Portfolio to continue.'}</p>{!busy&&<button type="button" className="governance-vote-primary" onClick={()=>void apply(mode||'local',false)}>Unlock Portfolio</button>}</div>}
